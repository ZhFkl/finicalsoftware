<template>
  <div class="my-wrapper">
    <div class="my-title">基于压缩映射的知识表达推理技术<i v-if="!originFlag" class="el-icon-back" @click="originFlag = true;showMetricFlag=false">返回</i>
    </div>
      ...
      <el-row type="flex" align="middle"  style="margin-bottom:10px;margin-top:10px">
        ...
        <el-col :span="5" :offset="1">
          <el-button type="primary" size="small" @click="showMetric"
            >查看指标</el-button
          >
          <el-button type="primary" size="small" @click="inferByStructure"
            >推理</el-button
          >
        </el-col>
      </el-row>
    ...
    getChoosePoint(val) {
      ...
      api.getNodeInfo({entity_id:val[0]._cfg.id, dataset:this.dataSource}).then(res =>{
        ...
        for(let i of Object.keys(res.data)){
          if(i==='embedding'){
            let tmpvalue={}
            tmpvalue['before']="["+res.data[i].before[0]+", ..., "+res.data[i].before[res.data[i].before.length-1]+"]"
            tmpvalue['after']="["+res.data[i].after[0]+", ..., "+res.data[i].after[res.data[i].after.length-1]+"]"
            this.nodeInfo.push({
              label:i,
              value:tmpvalue
            })
          ...
          if(i==="class"){
            this.nodeInfo.push({
              label:i,
              value:JSON.stringify(res.data[i])
            })
            continue;
          }
          this.nodeInfo.push({
    ...
    showMetric(){
        api.fetchInferMetric({
          ...
          for (let i = 0; i < res2.data.length; i++) {
            ...
            this.metricList=this.modelType==="HyperE"?this.metricListAll[1]:this.modelType==="TransE"?this.metricListAll[2]:this.metricListAll[0];
          }
          ...
          for (let j = 0; j < res.data[i]["entity"].length; j++) {
            if (uniqueNodes.indexOf(res.data[i]["entity"][j]["entity_id"]) < 0) {
              ...
              graphData.nodes.push({
                ...
                label: res.data[i]["entity"][j]["entity_name"]+(res.data[i]["weights"]?' - weights:'+res.data[i]["weights"][j]:""),
                size: 30,
                style: {
