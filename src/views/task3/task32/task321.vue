  ...
  <div class="my-wrapper">
    ...
    <div class="my-content">
      <el-row type="flex" align="middle"  style="margin-bottom:10px;margin-top:10px">
        <el-col :span="4">请选择依据的规则：</el-col>
        <el-col :span="10">
          <el-select size="small" style="width: 100%" v-model="ruleType">
            <el-option
        ...
        <el-col :span="5" :offset="1">
          <el-button type="primary" size="small" @click="loadData"
            >加载数据</el-button
          >
          <el-button type="primary" size="small" @click="handleTripleInfer"
            >推理</el-button
          >
        </el-col>
      </el-row>
      <el-row style="height:400px" v-if="ruleType">
        ...
        <!-- <el-col :span="11" :offset="1" style="height:100%">
          ...
          <tmp-graph :graphData="graphDataAfter" :hulls="hullsAfter" :ind="2" ></tmp-graph>
        </el-col> -->
      </el-row>

      
      <el-table :data="tableData" :span-method="objectSpanMethod" style="margin-top:10px;"  v-if="ruleType">
        <el-table-column prop="title" width="80">
          <template slot-scope="scope" >
            {{scope.row.title}}
          </template>
        </el-table-column>
        <el-table-column prop="before" label="持股关系">
          <template slot-scope="scope" >
            <div v-if="ruleType===1">
              <span style="font-weight:bold;">{{ scope.row.before.key }}</span>
              <span>&nbsp;股东：</span>
              <span style="font-weight:bold;" v-if="scope.row.before.highlight.length>0">{{ scope.row.before.highlight.join("；") }}；</span>
              <span>{{ scope.row.before.ordinary.join("；") }}</span>
            </div>
            <div v-else>
              <div v-for="item of scope.row.before" :key="item.key">
                <span style="font-weight:bold;">{{ item.key }}</span>
                <span>&nbsp;股东：</span>
                <span style="font-weight:bold;" v-if="item.highlight.length>0">{{ item.highlight.join("；") }}；</span>
                <span>{{ item.ordinary.join("；") }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="after" label="间接持股关系" v-if="showInferColumn">
          <template slot-scope="scope">
            <span style="font-weight:bold;">{{ scope.row.after.key }}</span><span>&nbsp;股东：</span>
            <!-- <span style="font-weight:bold;">{{ scope.row.before.highlight.join("；") }}；</span> -->
            <span>{{ scope.row.after.value.join("；") }}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
